...
private fun RecurrenceRule?.toFrequencyString(): String? =
    when (this) {
...
class TodoRepositoryImpl(
    ...
    private fun SelectWithDueDate.toDomain() =
        TodoItem(
            id = id,
            listId = list_id,
            title = title,
    ...
    private fun Reminder_list.toDomain() =
        TodoList(
            id = id,
            name = name,
            colorKey = color_key,
    ...
    private fun Reminder_list.toDto() =
        ListRowDto(
            id = id,
            name = name,
            colorKey = color_key,
    ...
    override fun observeToday(): Flow<List<TodoItem>> {
        ...
        return db.todoDbQueries
            .selectToday(start, end)
            .asFlow()
            .map { it.executeAsList() }
            .map { todos -> todos.map { it.toDomain() } }
            .flowOn(dbDispatcher)
    ...
    override suspend fun ensureInbox(): Either<TodoError, Long> =
        dbCommand("初始化收件箱失败") {
            ...
            db.todoDbQueries
                ...
                .executeAsList()
                .firstOrNull()
                ?.id ?: raise(TodoError.InboxNotFound)
        }
    ...
    override suspend fun updateList(
        ...
        dbCommand("更新列表失败") {
            ...
            db.transaction {
                val existing =
                    db.todoDbQueries.selectByIdForList(listId).executeAsOneOrNull()
    ...
    override suspend fun trash(id: Long): Either<TodoError, Unit> =
        dbCommand("移入垃圾箱失败") {
            db.transaction {
                val affected =
                    listOfNotNull(db.todoDbQueries.selectById(id).executeAsOneOrNull()) +
    ...
    override suspend fun restore(id: Long): Either<TodoError, Unit> =
        dbCommand("恢复待办失败") {
            db.transaction {
                val affected =
                    listOfNotNull(db.todoDbQueries.selectById(id).executeAsOneOrNull()) +
    ...
    override suspend fun deleteForever(id: Long): Either<TodoError, Unit> =
        dbCommand("彻底删除失败") {
            db.transaction {
    ...
    override suspend fun readOutbox(limit: Int): Either<TodoError, List<SyncRow>> =
        dbCommand("读取同步队列失败") {
            db.todoDbQueries
                .selectOutbox(limit.toLong())
                .executeAsList()
                .map { it.toSyncRow() }
        }

    /** 按水位线清除：seq <= upToSeq 的行视为已推送成功。 */
    override suspend fun clearOutbox(upToSeq: Long): Either<TodoError, Unit> = dbCommand("清理同步队列失败") { db.todoDbQueries.deleteOutboxUpTo(upToSeq) }
    ...
    override suspend fun applyRemoteUpsertList(row: ListRowDto): Either<TodoError, Unit> =
        dbCommand("应用远端列表失败") {
            db.transaction {
    ...
    private fun Raise<TodoError>.applyRemoteListDelete(
        rowId: Long,
        updatedAt: Long,
    ) {
    ...
    override suspend fun getSetting(key: String): Either<TodoError, String?> = dbCommand("读取设置失败") { db.todoDbQueries.getSetting(key).executeAsOneOrNull() }
