...
sealed interface TimelineScope {
    ...
    data class List(val listId: Long, val isInbox: Boolean) : TimelineScope
}
...
fun buildContextTimelineState(
    ...
    return when (scope) {
        TimelineScope.Workbench -> ContextTimelineState(
            title = "工作台时间线",
            subtitle = "所有待办的压力分布",
        ...
        TimelineScope.Scheduled -> ContextTimelineState(
            title = "计划时间线",
            subtitle = "未来安排分布",
            emptyText = "没有计划任务。",
            segments = listOf(
        ...
        TimelineScope.Completed -> ContextTimelineState(
            title = "完成回顾",
            subtitle = "最近完成的任务",
            emptyText = "还没有完成记录。",
            segments = listOf(
                segment("今天完成", completedParents.count { it.completedAt?.toLocalDateTime(timeZone)?.date == today }, TimelineTone.Success),
                segment("本周完成", completedParents.count { item ->
                    item.completedAt?.toLocalDateTime(timeZone)?.date?.let { completedDate -> completedDate.daysUntil(today) in 0..7 } == true
                }, TimelineTone.Info),
                segment("最近完成", completedParents.size, TimelineTone.Neutral),
            ),
        )
        TimelineScope.Trash -> ContextTimelineState(
            title = "垃圾箱摘要",
            subtitle = "可恢复和待清理的任务",
            emptyText = "垃圾箱为空。",
            segments = listOf(
...
private fun segment(label: String, count: Int, tone: TimelineTone): TimelineSegment =
    TimelineSegment(label = label, count = count, tone = tone)

private fun TodoItem.localDueDate(timeZone: TimeZone): LocalDate? =
    dueDate?.toLocalDateTime(timeZone)?.date

private fun TodoItem.isOverdue(today: LocalDate, timeZone: TimeZone): Boolean =
    localDueDate(timeZone)?.let { it < today } == true

private fun TodoItem.isDueOn(date: LocalDate, timeZone: TimeZone): Boolean =
    localDueDate(timeZone) == date

private fun TodoItem.isDueBetween(today: LocalDate, startOffset: Int, endOffset: Int, timeZone: TimeZone): Boolean {
...
private fun TodoItem.isDueAfter(date: LocalDate, timeZone: TimeZone): Boolean =
