...
class SyncCoordinatorTest {
    ...
    @Test
    fun drainPushesRowsAndClearsOutbox() =
        runTest {
    ...
    @Test
    fun drainKeepsOutboxWhenPushFails() =
        runTest {
            val repo = FakeTodoRepository()
    ...
    @Test
    fun drainCoalescesDuplicateUpsertsBeforePush() =
        runTest {
            val repo = FakeTodoRepository()
            repo.outboxState.value =
                listOf(
    ...
    @Test
    fun drainDoesNotCoalesceUpsertsAcrossDeleteBarrier() =
        runTest {
            val repo = FakeTodoRepository()
            repo.outboxState.value =
                listOf(
    ...
    @Test
    fun applyRemoteUpsertTodoDecodesPayload() =
        runTest {
            val repo = FakeTodoRepository()
            val coordinator = SyncCoordinator(repo, FakeSyncClient(), "device-a")
            val payload =
    ...
    @Test
    fun applyRemoteUpsertTodoIgnoresUnknownPayloadFields() =
        runTest {
            val repo = FakeTodoRepository()
            val coordinator = SyncCoordinator(repo, FakeSyncClient(), "device-a")
            val payload =
    ...
    @Test
    fun applyRemoteMalformedPayloadReturnsTransportError() =
        runTest {
            val repo = FakeTodoRepository()
            val coordinator = SyncCoordinator(repo, FakeSyncClient(), "device-a")
    ...
    @Test
    fun applyRemoteDeleteDelegates() =
        runTest {
            val repo = FakeTodoRepository()
            val coordinator = SyncCoordinator(repo, FakeSyncClient(), "device-a")
    ...
    @Test
    fun pullAppliesRemoteRows() =
        runTest {
            val repo = FakeTodoRepository()
            val client =
                FakeSyncClient().apply {
                    pullResult =
                        listOf(
    ...
    @Test
    fun pullSkipsMalformedRowButAppliesValidRows() =
        runTest {
            val repo = FakeTodoRepository()
            val client =
                FakeSyncClient().apply {
    ...
    @Test
    fun pullPropagatesTransportFailure() =
        runTest {
            val repo = FakeTodoRepository()
    ...
    private fun todoPayload(title: String) =
        syncJson.encodeToString(
            TodoRowDto(
                id = 0,
                listId = 1,
